...
- **Verifica compilazione:** Dopo modifiche a `src-tauri/`, eseguire `npm run check:rust` (o `cargo check` in `src-tauri/` se Cargo è in PATH; oppure `npm run tauri build`). Lo script `scripts/cargo-check.mjs` imposta CARGO_HOME e PATH come `run-tauri.mjs`, così la verifica funziona anche quando Cargo non è nel PATH della shell. Dettaglio: procedure [project-bootstrap](../procedures/workflow/project-bootstrap.md), [command-creation](../procedures/rust/command-creation.md).
