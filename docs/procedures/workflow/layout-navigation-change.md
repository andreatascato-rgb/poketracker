...
Definisce come aggiungere, rimuovere o modificare voci e sottovoci della sidebar e come mantenere coerenza con layout Top Bar + Sidebar + Area contenuto. **Layout e dimensioni:** Top Bar 48px, Sidebar 220px in PokeTracker (standard Poketrack 280px; scelta compatta), 48px collassata, area contenuto flex 1; scroll solo nell'area contenuto. Ordine voci: **Allenatore** (/profilo), **Statistiche** (/profilo/statistiche), **Pokedex**, **Editor**, **Salvataggi**, **Wiki** (con sottosezioni, in fondo); Impostazioni (Profili, Errori) dal dropdown Top Bar. Max due livelli. Registro errori in **Impostazioni → Errori** (/impostazioni/errori); vedi [notifications-and-error-archive.md](../../project/notifications-and-error-archive.md). Non esiste route né voce sidebar «Archivio». **Implementazione:** nav e layout shell sono in `src/routes/+layout.svelte` (markup inline); i componenti in `lib/components/layout/` non sono usati dal layout principale.
...
4. **Visibilità sottovoci:** visibili solo quando il padre è espanso (stato locale) **oppure** quando si è già in quella sezione. **"In sezione"** = il path corrente è l'href del padre, un href di una sottovoce, o un sotto-path di uno di essi (es. Wiki è "in sezione" su `/wiki` e `/wiki/pokemon`). Es.: `isExpanded = expandedSection === item.href || (expandedSection === null && inThisSection)`.
