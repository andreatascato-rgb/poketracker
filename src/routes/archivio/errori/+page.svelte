<script lang="ts">
  ...
  import { ChevronRight, AlertCircle, Eye, Trash2, Copy, User } from "@lucide/svelte";
...
{#if $errorArchiveEntries.length === 0}
  <div
    ...
    role="region"
    aria-label="Archivio errori vuoto"
  >
    <EmptyState
      ...
      ariaLabel="Stato vuoto: nessun errore in archivio"
    />
  </div>
{:else}
  <Card
    role="region"
    aria-labelledby="archivio-errori-title"
    class="w-full min-w-0"
  >
    <CardHeader>
      <CardTitle
        id="archivio-errori-title"
        class="text-xl font-semibold min-h-9 flex items-center gap-2"
      >
        <span class="text-muted-foreground">Archivio</span>
    ...
    <CardContent>
      <div
        ...
        role="region"
        aria-label="Tabella errori archiviati"
      >
        <table
          class="w-full min-w-[320px] text-sm"
          aria-label="Elenco errori archiviati"
        >
                    ...
                    <Tooltip>
                      <TooltipTrigger>
                        <Button
                          ...
                          aria-label="Elimina voce dall'archivio"
                        >
                          <Trash2 class="size-4 text-[var(--icon-destructive)]" aria-hidden="true" />
                        </Button>
                      </TooltipTrigger>
                      <TooltipContent side="top" sideOffset={6}>Elimina dall'archivio</TooltipContent>
                    </Tooltip>
                  </div>
                </td>
              </tr>
